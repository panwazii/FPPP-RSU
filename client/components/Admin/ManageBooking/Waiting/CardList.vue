<template>
  <div>
    <AdminManageBookingWaitingCreateQuotation
      v-if="bookingData"
      :bookingData="bookingData"
      :open="modalCreateQuotation"
      :create.sync="modalCreateQuotation"
    />
    <v-card class="rounded-xl mt-4">
      ...
      <v-card-actions>
        ...
        <v-btn
          ...
          elevation="0"
          class="rounded-xl"
          @click="modalCreateQuotation = true"
        >
          <v-icon class="mr-1">mdi-file-document-arrow-right</v-icon
          >ออกใบเสนอราคา
        </v-btn>
        <v-btn
          dark
          ...
          class="rounded-xl"
          @click="
            $router.push(`/admin/manage-booking/waiting/${bookingData.id}`)
          "
          >ดูรายละเอียด
        </v-btn>
      </v-card-actions>
